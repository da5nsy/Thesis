...
Data were calibrated in the following manner: the recorded RGB values os the observers' selections were bounded (values above 1 or below 0 were brought within range, with an `absolute' rendering intent) and converted via look-up-table to \gls{CIE} 1931 XYZ tristimulus values. From these, xy chromaticities and CIELAB values were computed (with the white point of the display, as loaded from the characterisation file, used as the white reference).

A set of data referred to as the `baseline' data was generated, where there was no observer input, to consider the range of possible responses that an observer could make.

\hl{insert figure, once I work out what the standard reporting style is going to be.}
