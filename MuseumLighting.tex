...
\textit{
\begin{enumerate}
\item ‘Adapt our eyes to the illuminant under test.
\item Look at a set of representative objects under it and accurately memorise their colours.
\item Adapt to the reference illuminant.
\item Look at the same objects under this second illuminant and compare the colours to the colours in our memory.’
\end{enumerate}}
\end{quote}
...
%The authors then pull reference from further studies focused on a range of other materials, and find low grade paper is actually an outlier in terms of wavelength sensitivity, with many other materials able to be grouped together in type of dependency (if not like sensitivity). Following Cuttle’s work11 to describe the pre-existing damage functions as simple mathematical relationships, comparison between the variety of newly created damage functions was possible.
